...
Simplify to annual sex-structured model - not required here
...
In the case of pollack, recruitment has been lower than expected for the last
15 years. The model is configured to estimate a 'regime' parameter, which
serves as an offset to R0 in the stock-recruit relationship. The value of this
parameter is 0.55, indicating that R0 is effectively reduced by that amount. Other
SR parameters are unaffected (steepness, sigma R). Here we test if the reference
points are robust to this level of reduced recruitment. Because steepness is
unaffected, this will only affect the risk to Blim and not FMSY itself.
...
rho = acf(maindevs,plot=F)$acf[2]
#sigmaR = out$sigma_R_in
sigmaR =  mean(an(out$sigma_R_info[1,"SD_of_devs"]))
...
```{r}
spars = FLPar(s=s,sigmaR=sigR, rho=rho,Fcv=Fcv,SSBcv=SSBcv)
...
Adjusted mse controls for sex-structured stocks - not required here
...
#theme(axis.text.x=element_blpol()) # HG edit: rotate axis instead
theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1)) +
xlab("Candidates") + guides(fill= guide_legend(ncol = 1))
pbp
...
write.csv(tab.pf,file=file.path("perftabs","ftune.tab.csv"),row.names = F)
