...
Ftgt = FLPar(c(Fmsy,Fx*Feq),params=c("Fmsy.om",paste0(Fx,".Feq"))) 
Ftgt
```
...
```{r}
Ftgt.tune = FLPar(c(Bftgt$F,Bftgt$B),params=c("Fmsy.om",paste0("Feq",Fx),"Bmsy.om",paste0("Bftgt",Fx))) 
...
```{r}
refpts = FLPar(Fmsy=Fmsy,Feq=Feq,Bmsy=Ftgt.tune["Bmsy.om"],Beq=Ftgt.tune["Bftgt1"],Blim = Blim,Bpa=Bpa,Btrigger=Btri.eq,B0=B0,R0=R0)
...
plot(stk.fmsy)+facet_wrap(~qname,scales="free")+
  theme_bw()+
...
```{r , fig.height=5,fig.width=8,warning=FALSE,message=FALSE, fig.cap = paste0("Advice Rule from EQSIM")}
 ...
 plotGFCM(fadv = Feq,btrigger =Btri.eq,bthr = -1,ftgt = Feq,btgt=df$Btgt[2],blim=an(refpts["Blim"]),fmin=0,bclose=0,kobe  =FALSE,text=F,rel=F,xmax=2.5)
```
