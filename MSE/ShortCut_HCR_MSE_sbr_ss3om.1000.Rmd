...
```{r , message=FALSE,warning=FALSE,fig.width=7,fig.height=7, fig.cap = paste0("Comparison of stock stock-recruitment function from ss3 and retuned single-sex with FLSRTMB ") }
...
```{r , message=FALSE,warning=FALSE,fig.height=6,fig.width=7, fig.cap = paste0("Seasonal  stock trajectories") }
...
```{r hc1, message=FALSE,warning=FALSE,fig.width=7,fig.height=7, fig.cap = paste0("Comparison of stock trajectories from ss3om and a backtest under the same Fbar") }
...
```{r , message=FALSE,warning=FALSE,fig.height=7,fig.width=7, fig.cap = paste0("Status Advice plot  showing stock trajectories of Recruitment, $SSB$, $F$, recruitment and $Yield$") }
...
save(mseargs, run,mp.eqsim, file="rdata/mp.eqsim.sbr.rda")
```


```{r fig9, fig.height=7,fig.width=7,warning=FALSE,message=FALSE, fig.cap = paste0("Initial OM and the MSE forecast horizon under a fixed $F_{tgt}$ rule")}

load(file="rdata/mp.eqsim.sbr.rda")
...
```{r, fig.height=7,fig.width=7,warning=FALSE,message=FALSE, fig.cap = paste0("Stock Status under a eqsim based Advice rule")}
thin = seq(1,it,5)
...
```{r , fig.height=5,fig.width=7,warning=FALSE,message=FALSE, fig.cap = paste0("Advice Rule from EQSIM, with $Ftgt$ denoting the true $Fmsy$ property of the SS3 benchmark model ")}
...
```{r , fig.height=8,fig.width=6,warning=FALSE,message=FALSE, fig.cap = paste0("Alternative HCR formulations with different tuning levels relative $F_{tgt}$, with $Ftgt$ denoting the true $Fmsy$ property of the SS3 benchmark model")}
...
```
These changes in parameters can simply be passed on to the existing `arule` to run the variations with `mps`.  This is the windows version and best to run outside the markdown (see MSE_loop.R).
...
```{r , fig.height=7,fig.width=7,warning=FALSE,message=FALSE, fig.cap = paste0("Initial OM and the MSE forecast horizon")}
...
plotAdvice(iter(pstks,thin))+facet_wrap(~qname,scales="free")+
  ...
  theme(axis.text.x = element_text(size=8, angle=90,vjust=0.5),legend.position="bottom", legend.box="vertical", legend.margin=margin(), legend.text = element_text(size=6), legend.title = element_blank())
...
```{r, fig.height=7,fig.width=7,warning=FALSE,message=FALSE, fig.cap = paste0("Median MSE forecast horizon under a fixed $F_{tgt}$ rule and 6 HCR scenarios")}
...
```{r, fig.height=9,fig.width=7,warning=FALSE,message=FALSE, fig.cap = paste0("Performance: Long-term performance evaluation")}
...
```{r , fig.height=5,fig.width=7,warning=FALSE,message=FALSE, fig.cap = paste0("Advice Rule PA,  with $Ftgt$ denoting the true $Fmsy$ property of the SS3 benchmark model")}
...
```{r fig5, fig.height=7,fig.width=7, fig.cap = paste0("MSE kobe plot Advice rules"), warning=FALSE}
