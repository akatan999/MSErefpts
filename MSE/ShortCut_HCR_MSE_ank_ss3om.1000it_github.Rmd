---
title: "Black-bellied anglerfish 27.8c9a: Short-Cut MSE approach for robustness tests of harvest control rules in sex-structured models"
...
However, there is increasing realization that the assessment estimates are imperfect. Therefore, ICES has implemented procedures to add uncertainty about the key quantities $F$ and $SSB$, where the error on $F$ is specified by a the random error term $Fcv$ and a first order autocorrelation parameter $Fphi$ and the precision of $SSB$ can specified by $SSBcv$ 
...
Note that the default setting assumes 1 year lag between data (reference) year and assessment (reporting) year and that the TAC is implemented the next year. In case the assessment is conducted in 2024, based on data from 2023 and the TAC is implemented for 2025.
...
library(doParallel)
#library(FLRef)
load("./rdata/inpMP.ank.1000.rdata",verbose=T)
length(hcrs$target)
# Run in batches
...
save(stks ,file=paste0("./rdata/mps.ftune.ank1000.rdata"))
